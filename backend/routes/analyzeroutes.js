...
router.post("/", async (req, res) => {
    try {
        const { nodes, edges, entryNodeId } = req.body;

        if (!nodes || !edges) {
            return res.status(400).json({ error: "Nodes and edges are required" });
        }
        const bottleneckResult = detectBottleneck(nodes, edges);
        const spofResult = detectSpof(nodes, edges, entryNodeId);
        const latencyRiskResult = detectLatencyRisk(nodes, edges, entryNodeId);
